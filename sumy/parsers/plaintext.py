...
from ..utils import cached_property, generate_sentences
...
class PlaintextParser(DocumentParser):
    ...
    @classmethod
    def from_string(cls, string, tokenizer, nlp):
        return cls(string, tokenizer, nlp)

    @classmethod
    def from_file(cls, file_path, tokenizer, nlp):
        with open(file_path, encoding="utf-8") as file:
            return cls(file.read(), tokenizer, nlp)

    def __init__(self, text, tokenizer, nlp=None):
    ...
    def document(self):
        current_paragraph = []
        paragraphs = []
        for line in generate_sentences(self._text, self.default_nlp, min_sen_len=3):
            if line.isupper():
