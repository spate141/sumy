...
def generate_sentences(text, nlp, min_sen_len=3):
    """
    Generate sentences using spacy.
    """
    doc = nlp(text)
    sentences = [{'size': len(i.text.split()), 'text': unidecode(i.text.strip(' '))} for i in doc.sents]
    sentences = [i for i in sentences if i['size'] >= min_sen_len]
    sentences = [i['text'] for i in sentences if i['text'].count(',') <= 4]
    return sentences
